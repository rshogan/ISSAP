...
2) Portable: the web app itself needs no Node.js, no npm and no build step. Two vendored libraries (Leaflet, SVG.js) are the only dependencies, checked into `web/vendor/`. A one-time Python pipeline (stdlib only) turns the source PDFs into the JSON the app reads; the app itself never needs Python once that data exists. The Electron desktop wrapper in `electron/` is strictly optional and additive -- `run.bat` and the browser path keep working unchanged, and nothing under `web/` may depend on Electron being present.
...
- **Liberation.** The colossi conquered and ravaged these nations; the player's half of the story is the opposite, so the UI vocabulary is liberation and remediation rather than conquest. A country is "liberated" once its boss is remediated; the game is "won" once all 4 are liberated. Save slots track this and show a badge. (The `localStorage` schema still keys this as `conquered`, and the code follows the key rather than the label, so existing saves keep working.)
